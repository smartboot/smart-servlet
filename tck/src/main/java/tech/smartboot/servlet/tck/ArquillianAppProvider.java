...
import tech.smartboot.servlet.util.WarUtil;
...
public class ArquillianAppProvider {
        ...
        if (baseDirVal != null) {
            ...
            if (baseDir.exists() && baseDir.isDirectory()) {
                ...
                if (targetDir.exists() && targetDir.isDirectory()) {
                    mavenTmpDir = new File(targetDir, "arquillian-smart-servlet-temp");
                    mavenTmpDir.mkdirs();
                }
            }
        }
    ...
    private final SmartEmbeddedConfiguration config;

    public ArquillianAppProvider(SmartEmbeddedConfiguration config) {
        this.config = config;
    }

    protected ServletContextRuntime createApp(Container containerRuntime, final Archive<?> archive) throws Exception {
